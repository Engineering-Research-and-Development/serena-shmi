  ...
  methods: {
    ...
    GetEnterprises() {
      this.fetchData('/enterprises').then((result) => {
        result.enterprises.forEach(enterprise => {
          if (enterprise.name != 'MIMOSA' || enterprise['@id'] != 'serena:enterprise/0') {
            var obj = { "id": enterprise['@id'], "name": enterprise.name, "url": this.LocalNifiResourceAddressFromURL(enterprise['@id']), "type": 'enterprise', "icon": 'fa fa-industry', "children": [] };
      ...
      this.fetchData(enterprise.url).then((result) => {
        result.sites.forEach(site => {
          if (this.ChildExists(enterprise.children, site['@id']) == -1) {
            var obj = { 'id': site['@id'], 'name': site.name, 'url': this.LocalNifiResourceAddressFromURL(site['@id']), 'type': 'site', 'icon': 'fa fa-map-pin', 'enterprise_index': index, 'children': [] };
      ...
      this.fetchData(site.url).then((result) => {
        result.segments.forEach(segment => {
          if (this.ChildExists(site.children, segment['@id']) == -1) {
            var obj = { 'id': segment['@id'], 'name': segment.name, 'url': this.LocalNifiResourceAddressFromURL(segment['@id']), 'type': 'segment', 'icon': 'fa fa-gears', 'enterprise_index': site.enterprise_index, 'site_index': index, 'children': [] };
      ...
      this.fetchData(segment.url).then((result) => {
        result.assets.forEach(asset => {
          if (this.ChildExists(segment.children, asset['@id']) == -1) {
            var obj = { 'id': asset['@id'], 'name': asset.name, 'url': this.LocalNifiResourceAddressFromURL(asset['@id']), 'type': 'asset', 'icon': 'fa fa-gear', 'enterprise_index': segment.enterprise_index, 'site_index': segment.site_index, 'segment_index': index, 'children': [] };
    ...
    GetChildren(value) {
      switch (value.type) {
        case 'enterprise':
          ...
          break;
        case 'site':
          var site_index = this.nav_dropdown_items[value.enterprise_index].children.indexOf(value);
          var site = this.nav_dropdown_items[value.enterprise_index].children[site_index];
          this.GetSegments(site, site_index);
          break;
        case 'segment':
          var segment_index = this.nav_dropdown_items[value.enterprise_index].children[value.site_index].children.indexOf(value);
          var segment = this.nav_dropdown_items[value.enterprise_index].children[value.site_index].children[segment_index];
    ...
    ShowDashboardCards(array) {
      ...
      array.forEach(element => {
        tmp.push({ "id": element.id, "name": element.name, "url": element.url, "type": element.type });
      });
    ...
    DashboardTableElement(item) {
      ...
      switch (item['@type']) {
        case 'Enterprise':
          ...
          tmp = { '@id': item['@id'], '@type': item['@type'], 'enterprise_id': item.enterprise_id, 'enterprise_type': item.enterprise_type.name, 'name': item.name, 'sites': children, status: 'Active' };
          break;
        case 'Site':
          children = [];
          for (i = 0; i < item.segments.length; i++)
            children.push(item.segments[i].name);
          tmp = { '@id': item['@id'], '@type': item['@type'], 'enterprise_id': item.enterprise_id, 'site_id': item.site_id, 'site_type': item.site_type.name, 'name': item.name, 'segments': children, status: 'Inactive' };
          break;
        case 'Segment':
          children = [];
          var sg_hyp_events = [];
          ...
          tmp = { '@id': item['@id'], '@type': item['@type'], 'segment_site': item.segment_site, 'segment_id': item.segment_id, 'segment_type': item.segment_type.name, 'name': item.name, "sg_hyp_events": sg_hyp_events, 'assets': children, status: 'Pending' };
          break;
        case 'Asset':
          children = [];
          var as_hyp_events = [];
          ...
          tmp = { '@id': item['@id'], '@type': item['@type'], 'asset_org_site': item.asset_org_site, 'segment_site': item.segment_site, 'segment_id': item.segment_id, 'asset_id': item.asset_id, 'asset_type': item.asset_type.name, 'serial_number': item.serial_number, 'long_description': item.long_description, 'name': item.name, "as_hyp_events": as_hyp_events, 'meas_locations': children, status: 'Banned' };
          break;
      }
      return tmp;
    },
